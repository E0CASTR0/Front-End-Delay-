...
export function Bonus() {
  return (
    <section id="bonus" className="relative overflow-hidden py-24 sm:py-28">
      <div
        aria-hidden
        className="absolute inset-0 bg-[radial-gradient(70%_60%_at_50%_0%,rgba(43,255,119,0.14),transparent_70%)]"
      />
      <div aria-hidden className="absolute inset-0 bg-noise" />

      <Container className="relative flex flex-col items-center text-center">
        <Reveal>
          <Eyebrow>Vantagens Extras</Eyebrow>
        </Reveal>
        <Reveal delay={80}>
          ...
          {BONUS_ITEMS.map((item, index) => {
            ...
            return (
              <Reveal key={item.label} delay={200 + index * 100}>
                <div className="group relative h-full overflow-hidden rounded-3xl border border-neon-500/40 bg-gradient-to-b from-neon-950/60 to-surface p-7 text-left shadow-card transition-all duration-300 hover:-translate-y-1 hover:border-neon-400 hover:shadow-glow">
                  <span className="inline-flex rounded-full border border-neon-500/50 bg-black/30 px-3 py-1 text-[10px] font-extrabold uppercase tracking-[0.2em] text-neon-400">
                    {item.label}
                  </span>
